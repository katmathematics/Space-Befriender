...
public class GameMaster : MonoBehaviour
{
    ...
    public void LevelController() {
        ...
        if (score == 0) {
            simpleBattalion.Add(Instantiate(simpleTank, new Vector2(0, -3.5f), Quaternion.identity));
        }
        else if (score == 1) {
            Destroy(simpleBattalion[0], 0f);
            simpleBattalion.RemoveAt(0);
            speedyBattalion.Add(Instantiate(speedyTank, new Vector2(0, -3.5f), Quaternion.identity));
        }
        else if (score == 2) {
            Destroy(speedyBattalion[0], 0f);
            speedyBattalion.RemoveAt(0);
            sillyBattalion.Add(Instantiate(sillyTank, new Vector2(0, -3.5f), Quaternion.identity));
        }
        else if (score == 3) {
            Destroy(sillyBattalion[0], 0f);
            sillyBattalion.RemoveAt(0);
            simpleBattalion.Add(Instantiate(simpleTank, new Vector2(-1, -3.5f), Quaternion.identity));
            simpleBattalion.Add(Instantiate(simpleTank, new Vector2(1, -3.5f), Quaternion.identity));
        }
        else if (score == 4) {
            Destroy(simpleBattalion[0], 0f);
            simpleBattalion.RemoveAt(0);
            speedyBattalion.Add(Instantiate(speedyTank, new Vector2(-1, -3.5f), Quaternion.identity));
        }
        else if (score == 5) {
            Destroy(speedyBattalion[0], 0f);
            speedyBattalion.RemoveAt(0);
            sillyBattalion.Add(Instantiate(sillyTank, new Vector2(-1, -3.5f), Quaternion.identity));
        }
        else if (score == 6) {
            Destroy(simpleBattalion[0], 0f);
            simpleBattalion.RemoveAt(0);
